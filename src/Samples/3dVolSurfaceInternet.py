...
df = pd.DataFrame()
df2 = pd.DataFrame(columns=["TTM", "Strike", "ImpliedVol"])

vd = VolatilityMatrixData(67604447, 67604448, 1)
vd.LoadVolatility()
...
data['TTM'] = data.apply(lambda row: ttm(row.TenorPeriod), axis=1)
...
location = os.path.join(tempfile.gettempdir(), '3dVol.png')
plt.savefig(location)
print('<img src="3dVol.png" />')
